...
func initializeHandler(w http.ResponseWriter, r *http.Request) {
	...
	htmlMap = make(map[string]string, 10000)
	rows, err := db.Query(`SELECT keyword FROM entry ORDER BY CHARACTER_LENGTH(keyword) DESC`)
	keywords = make(map[string]bool, 10000)
	keywordList = make([]Keyword, 0, 10000)
	panicIf(err)
	for rows.Next() {
		var k string
		err := rows.Scan(&k)
		...
		if _, ok := keywords[q]; !ok {
			keywords[q] = true
			keywordList = append(
				keywordList,
				Keyword{Value: q, Length: len(k), Hash: fmt.Sprintf("isuda_%x", sha1.Sum([]byte(q)))},
			)
		}
	}
	rows.Close()
	...
	keywordsString = strings.Join(keys, "|")
	reg = regexp.MustCompile("(" + keywordsString + ")")

	starsMap = make(map[string][]Star, 100)
...
func keywordPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		if _, ok := keywords[q]; !ok {
			...
			keywordList = append(
				keywordList,
				Keyword{Value: q, Length: len(keyword), Hash: fmt.Sprintf("isuda_%x", sha1.Sum([]byte(q)))},
			)
...
func starsHandler(w http.ResponseWriter, r *http.Request) {
	keyword := r.FormValue("keyword")
	starMu.RLock()
	defer starMu.RUnlock()
	if ss, ok := starsMap[keyword]; ok {
...
func starsPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		starMu.Unlock()
		starIndex++
		starsMap[keyword] = append(starsMap[keyword], Star{
			ID:        starIndex,
			Keyword:   keyword,
			UserName:  user,
			CreatedAt: time.Now(),
...
func htmlify(w http.ResponseWriter, r *http.Request, content string) string {
	...
	k := keywordsString + "-" + content
	if s, ok := htmlMap[k]; ok {
		return s
	}
	for _, v := range keywordList {
		content = strings.Replace(content, v.Value, v.Hash, -1)
	}
	content = html.EscapeString(content)
	for _, v := range keywordList {
		u, err := url.Parse(baseUrl.String() + "/keyword/" + pathURIEscape(v.Value))
		panicIf(err)
		link := fmt.Sprintf("<a href=\"%s\">%s</a>", u, html.EscapeString(v.Value))
		content = strings.Replace(content, v.Hash, link, -1)
	}

	ret := strings.Replace(content, "\n", "<br />\n", -1)
